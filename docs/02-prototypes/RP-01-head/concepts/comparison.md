...
| Physical baseline | ~95 × 150 × 115 mm nominal complete head within a 90–100 × 145–155 × 110–120 mm validation band; `../material-finish-mass-decision.md` provisional ~490 g at 1.2 mm PLA; 60 mm neck allocation |
...
| τ_peak margin per axis (from physics.md) | Pending the per-axis downstream mass tree around the current ~490 g `E` build-up, candidate gimbal-centre sweep and separate transient/RMS screen in `physics.md`. Old 250 g / 0.2 N·m conclusions are inadmissible. A low-axis inverted-pendulum arrangement is not the reference geometry. | |
