...
| Field | Value |
|---|---|
| Status | **Registered Part-1 fault-definition baseline v0.2 — builder-approved 2026-09-15; no injection performed and numeric G04 thresholds remain unregistered** |
| Owner | Project builder |
| Created | 2026-09-08 |
| Authority | `../../01-system/risk-prototype-plan.md` v1.12 §RP-02 (inject "high-level process loss, internal-link loss/corruption or staleness, controller restart, sensor absence, network loss, and one allowed subsystem brownout at a time"); RP02-G04 |
...
|---|---|---|
| Time from injection to `BRAKE` onset | ≤ 200 ms (heartbeat-mediated); ≤ 1 control tick (locally detected: motor domain, servo/safety sensor/base controller) | F-01…F-03, F-06, F-08, F-12, F-19, F-22 |
...
Injections are ordered by increasing authority: F-15, F-14 and F-04 first; then F-05, F-07, F-10, F-20 and F-21; then F-17 app-session loss; then F-01, F-02, F-03, F-06 and F-08 with representative servo load; then F-19 and F-22 on the guarded RP-03 rig; then F-11/F-16 supply ramps; then F-18 after the charge path exists. F-12/F-13 run **every session** and are additionally scored under `CC-06` and `CC-10A` when drive hardware exists.
