...
| `BD-02` | **Drive stage is fed directly from `PB-MOTOR` at pack-native voltage; no drive converter.** One DRV8874 per motor, as already modelled (Pololu 4035 carriers stay purchased modules for V1). Motor SKU lock is **open**; Pololu #4804, HP 6 V / 34.014:1 / encoder, is the preferred prototype candidate (`BA-04`). Its 240 rpm maximum-efficiency point at 0.127 N·m clears the 159 rpm / 0.110 N·m paper screen, but 6 A extrapolated stall requires a verified hardware current limit and bench proof. C3 limits motor-terminal voltage and speed on the 2S bus | 2S is 6.0–8.4 V. Removes a converter and keeps regeneration returning to the pack through a bidirectional gate | Exact #4804 cannot be supplied to India, verified current limiting is incompatible with launch torque, or measured speed at the 6.0 V floor misses the 0.70 m/s target on the 84 mm wheel |
...
| `BA-04` | **SKU lock OPEN; Pololu #4804, HP 6 V / 34.014:1 / quadrature encoder, is the preferred prototype candidate**, per [RP-03 lock status](../RP-03-locomotion/gearmotor-sku-decision.md). Manufacturer: 290 rpm no-load, 240 rpm at 0.127 N·m maximum-efficiency point, 6 A extrapolated stall | Fab.to.Lab lists #4804 in India but variant-specific stock is unverified. Obtain a quote for two plus spare before locking the SKU. Verify the DRV8874 hardware current limit against launch demand and the 5 A bench supply; its 12.5 mm shaft and official drawing are in the CAD (RP-06 `RP03-CAD-10`, 2026-09-26); confirm on received samples, then bench test. A limit of about 3 A gives about 0.30 N·m and also caps regeneration at the 3 A bus requirement. NFP `-0685` remains a comparison with no verified India source |
...
**Answered 2026-09-26** (RP-05 `BD-A06`, [`peripheral-selection.md`](../RP-06-cad/peripheral-selection.md) §2.5): MAX98357A on `PCB-05`, 2.5–5.5 V on this 5 V rail directly; 1.75 W into the 8 Ω Visaton K 50 WP, about 0.4 A average and 0.7 A crest from 5 V (`E`); 2.4 mA idle, 0.6 µA shutdown (`D`); internal click/pop suppression, 7 ms turn-on; `SD_MODE` held low by 100 kΩ so the amp is **off by default**, enabled by C0 GPIO23 `AMP_SD` at 3.3 V logic; filterless spread-spectrum output; mics run from their own Pi-header 3.3 V domain with 80 dB PSR. The 2.0 A `TPS259474L` setting has margin.
...
| Drive motor: preferred prototype candidate Pololu #4804; obtain exact-variant India stock/quote, verify current limiting for its 6 A extrapolated stall, reconcile the 12.5 mm shaft and official drawing with CAD, then measure current/speed. See [SKU decision](../RP-03-locomotion/gearmotor-sku-decision.md) | Supplier, bench, CAD | `BD-02`, RP-03 |
