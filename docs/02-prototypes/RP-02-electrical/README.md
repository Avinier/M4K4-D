...
| Field | Value |
|---|---|
| Status | **Planning open, 2026-09-08.** No state registered, no gate registered, no run executed. Phase A work is live; Phase B waits on RP-01's servo selection and an SBC candidate; Phase C waits on the battery gate |
...
| [State register](state-register.md) | The registered concurrent states `S00…S17` and the proposed 20-minute mixed-duty cycle `MD-01` — RP-02's storyboard | A |
...
| [Link contract](link-contract.md) | SBC ↔ C2 ↔ display message set, framing, expiry, heartbeat, recovery semantics, bandwidth and timing candidates — v0.1, the artifact that outlives the prototype | A |
...
| Control topology, C1/C2, C2 module | [`control-topology-options.md`](../../01-system/control-topology-options.md) v0.9 | RP-02 closes this study into ADR-12; it does not fork it |
...
4. Implement `timebase.md` §3 and `link-contract.md` v0.1 on the DevKitC-1 twin against a laptop; run the §6 offset-error measurement as `RP02-EXP-exploratory` runs. This is software and does not wait on any purchase.
