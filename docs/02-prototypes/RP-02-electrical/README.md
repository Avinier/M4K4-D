...
| [Custom board specifications](board-specs.md) | Working (unregistered) per-board design, v0.19 (2026-09-25): parts, values, sizes and evidence lists for the four custom power/safety PCBs (pack protection, charge and system power, motor gate and head/drive distribution, branch converters) and the watchdog block; no gate registered, no SKU frozen, no purchase authorized | A |
| [Registration draft `RP02-P2-REG-04`](registration-draft-reg-04.md) | **Draft, not registered.** Proposed supersession of the `PB-COMPUTE` 5.1 V nominal, the `PA-14` `OFF` wording and the arm-latch clear rule, with the edits to make on approval and an approval block for the builder | A |
