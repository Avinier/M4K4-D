...
| HEAD-CAD-12 | **Modelled in Layout 04 (2026-09-26); bench evidence remains** | Status light is a Worldsemi WS2812B-2020-V6 on a 5 × 5 × 0.8 mm carrier (`PCB-08`) inside the existing `M007` reserve, behind the unchanged Ø3.4 × 2.9 light pipe; driven by D1 GPIO6 at 3.3 V; three AWG30 leads turned 90° at the carrier. Evidence: [`../peripheral-selection.md`](../peripheral-selection.md) §3 |
...
**Current CAD pass, audited 2026-09-13:** [Layout 03](layout-03/README.md) packaging remains the accepted direction (104 mm stern, 3 mm camera gap, A0), but the geometry is not ready to fabricate. The closure audit confirmed two retainer-screw collisions, hard stops at rather than beyond usable travel, high pitch-stiffness risk, secondary roll-saddle stiffness risk, trial-only bearing seats, no physical A0 trim and an undocumented yaw/roll sign inversion. See [verification](layout-03/review/verification.md). Concept A only.
...
| Exact axis coordinates and achieved balance | A0 target is selected; derive coordinates from per-axis mass, 3D centre of mass and inertia, but treat micron residuals as audit output. Provide an adjustable/measurable physical trim method and verify achieved offsets with `W` hardware. |
| Servo selection | Actual trajectory torque/speed demand plus thermal, reversal and settling evidence |
| Bearing and printed interfaces | Select real bearing/insert SKUs; remove the two confirmed retainer-screw collisions; update seats/preload/retention; move hard stops beyond usable travel and check pin/root/load-path strength; verify PLA creep. |
| Structural dynamics | Stiffen and verify the open pitch frame against 30/40 Hz and the roll saddle against 25/30 Hz under representative load; simple screens (~6–9 Hz pitch, ~21 Hz roll saddle) are risks, not final modal predictions. |
| Motion sign interface | Preserve storyboard signs; document/test `P_cad=+P_story`, `Y_cad=-Y_story`, `R_cad=-R_story` through CAD, calculation export and firmware. |
...
| 2026-09-26 | HEAD-CAD-12 in Layout 04: `status_LED_PCB08_carrier_5x5` (X −8…−7.2) and `status_LED_WS2812B_2020` (X −7.2…−6.36) added inside `addressable_status_LED_package_reserve`, 1.36 mm air gap to the diffuser. No aperture, diffuser or mass change (`M007` stays the 5 g `E` allowance). The LED lead route to D1 stays a keep-out. |
