...
def write(parts=None):
    ...
    result=dict(layout='04',units='mm',frame='+X forward,+Y left,+Z up; origin front/bottom centre',pose='neutral',axes=m.AXES,parts=rows,physical_bounds=dict(min=lo,max=hi,size=[hi[i]-lo[i] for i in range(3)]),camera_display_gap_mm=m.CAMERA_BOTTOM-74,crown_height_mm=m.CROWN_H,yoke=dict(visible_below_head_mm=-m.YAW_DISC_TOP_Z,pitch_to_disc_top_mm=m.PITCH_Z-m.YAW_DISC_TOP_Z,yaw_interface_z_mm=m.YAW_INTERFACE_Z,sweep_floor_z_mm=m.SWEEP_FLOOR_Z,turntable_disc_diameter_mm=2*m.YAW_DISC_R),note='AABB centres, not centres of mass. Catalog camera/servo use their actual placed manufacturer solids in this log. Annotation geometry uses this same measure function on the actual displayed part.')
    here=Path(__file__).parent;(here/'dimensions.json').write_text(json.dumps(result,indent=2)+'\n')
    fmt=lambda v:' × '.join(f'{x:.3f}' for x in v)
    lines=['# Layout 04 dimensions','',result['frame'],'',f"Physical envelope ΔX × ΔY × ΔZ: {fmt(result['physical_bounds']['size'])} mm (includes {-m.YAW_DISC_TOP_Z:g} mm neck to the flush turntable top). Crown {m.CROWN_H:g} mm; camera/display gap {m.CAMERA_BOTTOM-74:g} mm. Rear section {m.HELMET_REAR_WIDTH:g} × {m.HELMET_REAR_TOP-m.HELMET_REAR_BOTTOM:g} mm (Z{m.HELMET_REAR_BOTTOM:g}…{m.HELMET_REAR_TOP:g}); full face/ears section unchanged.",'',f'Visible yoke {-m.YAW_DISC_TOP_Z:g} mm above a Ø{2*m.YAW_DISC_R:g} flush turntable; pitch axis to disc top {m.PITCH_Z-m.YAW_DISC_TOP_Z:.3f} mm; A+ sweep floor Z{m.SWEEP_FLOOR_Z:g}. A0: `{m.AXES}`.','', '| Named component | Frame | Centre global XYZ (mm) | ΔX × ΔY × ΔZ (mm) |','|---|:---:|---|---|']
