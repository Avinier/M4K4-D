...
if '--neutral' in sys.argv:poses=[(0,0)]
for r,p in poses:
...
result=dict(method='Conservative 1:1 package BREP overlaps; 7 roll × 8 pitch grid clamped onto the A+ motion envelope, distinct from relief design samples. Intended bearing/shaft contacts omitted.',axes=m.AXES,poses=len(poses),sweep_floor_z=m.SWEEP_FLOOR_Z,pairs_per_pose=len(pairs),hits=hits,worst_by_pair=worst,static_package_hits=static,active_area_excluded_mm3=screen_excluded,outer_cap_face_area_mm2=outer_caps,lowest_sampled_shell_z=min(minz),bounds=dimensions,limitations=['No continuous sweep, cable flex, optical field of view, body/yaw model, manufacturing tolerance or structural certification.','Servo references remain unselected. Board depth, LED installed package, connector and service envelopes are assumptions.'])
result['method']=f'Conservative 1:1 package BREP overlaps; {len(poses)} discrete poses (7 roll × 8 pitch clamped onto the A+ envelope). Intended bearing/shaft contacts omitted.'
