"""Layout 04 exhaustive cross-frame sampling and targeted physical requirements."""
...
if '--service-only' in sys.argv:
    ...
    assert not prior['motion_hits'] and not prior['harness_pinches']
    (HERE/'review/motion-before-service-path-repair.json').write_text(json.dumps(prior,indent=2)+'\n')
# A+ firmware envelope: the 7 x 8 roll/pitch grid is clamped onto its boundary.
...
stops=[]
for axis,limits,pin,fixed in [('roll',list(m.ROLL_STOP),'roll_hard_stop_pin',['bearing_cartridge_trial','roll_bearing_retainer_39.5']),('pitch',list(m.PITCH_STOP),'pitch_hard_stop_pin',['yaw_yoke_leg_-55'])]:
    for angle in limits:
        for extra in [0,1]:
