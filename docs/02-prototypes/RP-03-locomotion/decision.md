...
| Drive motor | **SKU lock OPEN.** Pololu #4804, HP 6 V / 34.014:1 / encoder is the preferred prototype candidate, per [SKU lock status](gearmotor-sku-decision.md). NFP `-EN-0685` remains a technical comparison; `D02` 176 rpm remains a reference, not an electrical alternate | Ø84 4 mm D-hub; 159.2 rpm at 0.70 m/s; #4804 240 rpm at 0.127 N·m maximum-efficiency point and 6 A extrapolated stall; 6 V motor-terminal control on the 2S bus | Confirm exact #4804 India stock, quantity and lead time, verify hardware current limiting, then drawing/CAD reconciliation, receiving measurements, P01–P06 and scored G01/G02/G06 before installed-part freeze |
...
| Base IMU | Runtime from Phase B (BD-03 working assumption); Phase A may log as bench instrument. **Article working-selected 2026-09-26:** ICM-42688-P on its own `PCB-07` board (16 × 20 mm, SPI + INT1 + INT2/CLKIN on JST-SH 8-pin, two M2 into the chassis deck crossbar); bench article any breakout with SPI and INT1 out (MIKROE-4237 where stocked) ([evidence](../RP-06-cad/peripheral-selection.md) §4) | SPI+INT reserved on the pin map; rigid base mount; not in the head | G01 lift / G05 pickup. `PCB-07` layout, mounting stiffness on the printed deck and bias vs temperature open |
...
No paper gate passed. No physical gate passed. No purchase authorized. **V1 front support is `D21` (BD-08, baseline v1.12, SCOPE-09 v1.3).** [Pololu #4804](gearmotor-sku-decision.md) is the preferred prototype motor candidate within Set A; **the gearmotor SKU lock is open pending exact India stock, current-limit proof and geometry checks.** Concept A chassis, DRV8874-class and S01/S04/S06/S07 remain leads. `D02` 176 rpm remains a reference-only comparison. `D20` is the caster comparison swap, not waived. ADR-04 is not closed. Parts 1–5 are design-definition; the 2026-09-18 brief-gap close (`RP03-P1-REG-02` … `RP03-P4-REG-02`) filled operating cases, remaining physics, complete drive sets, TTL candidates, cutoff-coast, and the `CA-14` header. Phase A bench work on C3 may begin when the driver evaluation board and reference motor are on the bench; it is not a scored RP-03 run. CAD pass 1 and the 240 fps mock-up remain open.
