...
| Field | Value |
|---|---|
| Status | **Living — Part-1 `CC/LP` vocabulary is registered under `RP02-P1-REG-01`, branch mapping/implementation under `RP02-P2-REG-01…03`, and compute/control identities under `RP02-P3-REG-01/02`; `LG-04`/`LG-10` envelopes refreshed from RP-03 `physics.md` as `E`/`U`. Every numeric row remains `E`, `D`-cited or `U`. No `W` evidence exists.** |
| Version | 0.14 |
...
| `LG-04` drive | 0 (inhibited) | 2–8 W in motion for two small encoder gearmotors | 20–40 W for both at stall/reversal; 1.5–3 A per motor class | `LP-04-LAUNCH/REV/BRAKE`; signed regenerative current required (0.3–1.5 A class `E`, opposite polarity) | `E` (class) | RP-03 `physics.md` 2026-09-17: mechanical floor load 0.4–1.4 W; gearbox/driver bring electrical into 2–8 W. JGA25-class D02 is a **reference unit, not a freeze**. Korad 5 A: Phase B stall/reversal is **per-motor**; both-motor composites wait Phase C | RP-03; substitute profile in RP-02 until then |
