...
### MEM-20260914-01 — Head component review and body-SBC suggestion
**Type / Status / Governs:** REVIEW FINDING / PROVISIONAL / `control-topology-options.md` v0.12, `display-candidate-study.md` v0.8, `camera-candidate-study.md` v0.4, `candidate-sourcing-matrix.md` v0.20, `power-energy-ledger.md` v0.4, RP-02 `decision.md` candidate register and `link-contract.md`

A from-all-angles review of the selected head parts found no reason to reopen any lock: the ESP32-S3-Zero (C2), the Waveshare ESP32-S3-LCD-4.3 (face) and the Camera Module 3 Wide (camera) stand. Findings recorded as **suggestions, not decisions**: (1) the display's realistic sustained rate is 30–40 fps, not 50–60, so register the refresh gate accordingly; (2) IMX708 support binds the body SBC to Raspberry Pi in practice; (3) **Raspberry Pi 5 2 GB is the suggested SBC lead candidate** at ≈ ₹8,300–8,700 in India after the 2026 DRAM price rises — the 8 GB is ≈ ₹20k and Pi 4 is now cost-dominated; (4) four PDM mics need a front end on a Pi; (5) a C2-relay display link is worth evaluating; (6) C2 flashing needs no BOOT hold while the link stays on UART. Nothing selected, nothing purchased, no gate registered. Purchase authorization remains the project builder's.

### MEM-20260915-01 — RP-02 Part-1 sleep, mode-selection and charging decisions
**Type / Status / Governs:** DECISION / CURRENT / RP-02 `state-register.md` v0.3, `load-model.md` v0.2, `power-architecture.md` PA-09

Builder decisions: the camera is off while Makad sleeps; all head-servo torque is off and the head rests passively down; V1 Floor/Table mode is selected manually in the control app, accepted and latched locally for the session, with boot/reset/app-link loss returning to motion inhibit; and the display remains on while charging while camera, ordinary SBC interaction, audio and motors are off. AC mains never enters Makad: Part 2 must select a certified external AC/DC adapter, keyed low-voltage charge input, and chemistry/S-count-specific onboard charger with supported power-path/load-sharing behaviour. The complete Part-1 case/profile and `MD-01` registration remains pending.

### MEM-20260915-02 — RP-02 state coverage separates runtime state from qualification cases
**Type / Status / Clarifies / Governs:** DESIGN + CORRECTION / CURRENT DRAFT / the unregistered v0.1 `S00…S17` model / RP-02 `state-coverage-matrix.md`, `state-register.md`, `load-model.md`, `fault-matrix.md`

Runtime condition is the orthogonal vector `<OM, EN, BS, HL[], {EV}>`; a `CC` is not a runtime state enum but a frozen qualification recipe that selects a meaningful vector/transition, loads, expected terminal state and evidence. The coverage audit split Floor/Table sleep (`CC-02F/T`), tabletop permitted/rejected behaviour (`CC-12A/B`) and head/drive low-energy cases (`CC-13H/D`); added normal orderly-shutdown `CC-16`; and added app-session-loss `F-17` plus charge-path/status-loss `F-18`. All remain unregistered candidates.

### MEM-20260915-03 — RP-02 foundation/system situation audit adds person and action overlays
**Type / Status / Supersedes / Governs:** DESIGN + CORRECTION / CURRENT DRAFT / the runtime-vector completeness claim in MEM-20260915-02 / RP-02 `operating-situations.md`, `state-coverage-matrix.md`, `state-register.md`, `load-model.md`, `fault-matrix.md`; `power-energy-ledger.md` v0.6

A line-by-line audit of every `00-foundation` document plus the operating/failure and prototype situations in `01-system` showed that `<OM, EN, BS, HL[], {EV}>` was not complete enough. Runtime condition is now `<OM, EN, BS, PC, AL[action], HL[subsystem], EV[]>`: `PC-00…06` preserves selected-person continuity through loss/reacquisition, and per-action `AL-00…06` prevents pending, cancelling, completed and failed work from being confused with the current face/behaviour. The audit also made idle aliveness, false wake, ambiguity/rejection, cancellation/late duplicate, distinct Come and Follow, obstacle response, tabletop edge intervention and denied-channel composition explicit through `EV-17…22`, new/lettered `CC` cases and `F-19…22`. Spatial/directional hearing remains Candidate, not Core. All records remain draft and unregistered; this correction creates no measured evidence or component selection.

### MEM-20260915-04 — RP-02 Part-1 state/load baseline registered
...
### MEM-20260916-05 — RP-02 compute/control architecture registered; Part-3 definition complete
**Type / Status / Governs:** DECISION / CURRENT / `RP02-P3-REG-01`; RP-02 `compute-control-architecture.md` v1.0, `compute-control-component-screen.md` v1.0, `link-contract.md` v0.3; `control-topology-options.md` v0.14

Builder directed Part 3 to be completed directly from the registered state/load and power baselines. The architecture fixes C0 Raspberry Pi 5 application ownership, separate ESP32-S3 C2 head and C3 base safety ownership, C2-relayed display traffic, full-duplex differential production UART links, C0 process/authority-lease boundaries, three watchdog layers, two-phase fresh arming, radio-off MCU service policy, the official Pi 5 Active Cooler and the ESP32-S3-DevKitC-1-N8R8 as the C3 prototype board. ROS 2/micro-ROS are not V1 safety dependencies. Exact link/watchdog suffixes and carrier circuits, storage, C3 pins, motor driver/sensors, timeouts, purchases and every G04/G05 result remain open; `F-23…30` add required qualification cases without claiming a run or ADR closure.

### MEM-20260916-06 — C3 prototype corrected to DevKitC-1-N8 after GPIO audit
**Type / Status / Supersedes / Governs:** CORRECTION / CURRENT / the C3 N8R8 suffix in MEM-20260916-05 / `RP02-P3-REG-02`; RP-02 `compute-control-architecture.md` v1.1 and `compute-control-component-screen.md` v1.1

The official ESP32-S3-DevKitC-1-N8 supersedes N8R8 as the C3 prototype board. C3 does not need PSRAM, while N8R8's octal PSRAM consumes GPIO35–37; N8 preserves those pins for the RP-03 motor/safety I/O budget. N8R8 is only a conditional substitute after a frozen pin map proves at least three spare safe GPIO and the board delta is requalified. No purchase or gate outcome follows.

### MEM-20260916-07 — RP-02 Part-3 total-installed-cost sourcing review
**Type / Status / Governs:** REVIEW FINDING / CURRENT / RP-02 `compute-control-component-screen.md` v1.2; `compute-control-architecture.md` v1.2 implementation lead; `candidate-sourcing-matrix.md` v0.25

The cost-down pass retained the selected Raspberry Pi 5 2 GB, official Active Cooler, C2, C3 and no-touch D1. Raspberry Pi 5 1 GB is rejected for the first build because the captured ₹555 saving halves memory; a constrained 2 GB run is only a pre-screen and actual 1 GB hardware would still require qualification. Official Raspberry Pi 32 GB A2 becomes the storage lead; SanDisk High Endurance 32 GB is an endurance comparison, **not** a low-cost lead (the ₹679 staging-store signal was invalid); 64 GB is a measured-capacity fallback. THVD1451D SOIC supersedes MAX3490E only as the full-duplex differential-link **implementation lead**: it preserves the registered topology, improves the protection/common-mode feature set, and reduces four endpoint ICs from roughly ₹3,910 to ₹1,497 at the dated single-unit price snapshot. Renesas ISL83077E is a ~₹742/four-IC challenger only if protection, startup and installed-harness tests justify a substitution; its ±15 kV HBM claim is not TI's ±18 kV IEC-contact claim. The TI VSON suffix is a production cost option only with qualified assembly. TPS3436-Q1 remains selected because the small TPS3430 saving does not repay package/qualification cost. No purchase, gate, storage selection, exact watchdog/transceiver suffix or registered architecture rule changed.

### MEM-20260917-01 — RP-03 locomotion folder planned
